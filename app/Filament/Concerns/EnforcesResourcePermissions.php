 ...
 *
 * OrcaTech demo packages: a resource may declare `protected static ?string
 * $orcaTechFeature = 'feature_key'` to hide it (and block its pages) while
 * that feature is locked in the active demo package. Direct URL access is
 * redirected to the upgrade preview by OrcaTechFeatureGate middleware.
 */
trait EnforcesResourcePermissions
{
    public static function orcaTechFeature(): ?string
    {
        return property_exists(static::class, 'orcaTechFeature')
            && is_string(static::$orcaTechFeature)
            ? static::$orcaTechFeature
            : null;
    }

    protected static function orcaTechFeatureLocked(): bool
    {
        return static::orcaTechFeature() !== null
            && ! Feature::available(static::orcaTechFeature());
    }
    ...
    {
        if (static::orcaTechFeatureLocked()) {
            return false;
        }
