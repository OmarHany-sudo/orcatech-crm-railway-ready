...
Railway is the recommended environment for the public OrcaTech CRM demo. The project includes a PHP 8.5 Docker image, dynamic `PORT` handling, a safe `/health/live` endpoint, Railway configuration, and a manual demo initialization command. Use [`RAILWAY_DEPLOYMENT.md`](RAILWAY_DEPLOYMENT.md) for the current one-service setup with Railway MySQL, required variables, first-run migration/seeding, logging, and storage guidance. The demo keeps queues synchronous and does not require Redis, Horizon, Reverb, Octane, or a separate worker service.
...
- [`RAILWAY_DEPLOYMENT.md`](RAILWAY_DEPLOYMENT.md) — Railway public demo deployment guide.
